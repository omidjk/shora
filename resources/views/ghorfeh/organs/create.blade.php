        ...
        <div class="card-body">
            ...
            <form method="post" action="{{ route('organs.store') }}">
                <div class="form-group">
                    ...
                    <select id="group_name" class="form-control" name="group_name">
                        <option value='0'>-- انتخاب گروه --</option>
                        @foreach($groupnamedata['data'] as $key => $groupnamedata)
                            <option value='{{ $groupnamedata->group_name }}'>{{ $groupnamedata->group_name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label for="code">کد ارگان :</label>
                    <input type="text" onkeypress="return isNumberKey(event)" minlength="3" maxlength="5" class="form-control" name="organ_code"/>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="organ_address"/>
                </div>
                <td><a href="{{ route('organs.index')}}" class="btn btn-primary">بازگشت</a></td>
                <button type="submit" class="btn btn-primary">ایجاد ارگان</button>
            </form>
        </div>
    </div>
@endsection